...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const { id, robot_id, advice_number, content, is_exception } = body;

        if (!id || !robot_id || advice_number === undefined || !content) {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .upsert({
                id,
                robot_id,
                advice_number: parseInt(advice_number),
                content,
        ...
        try {
            ...
            await fetch(`${origin}/api/notifications/broadcast`, {
                ...
                body: JSON.stringify({
                    title: `Nuevo Consejo (${robot_id.toUpperCase()}) 💡`,
                    body: content || 'Se ha registrado un nuevo consejo operativo.',
                    url: '/'
                })
            });
        } catch (notifErr) {
        ...
        console.error('Error in POST /api/admin/advises:', err);
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        ...
        console.error('Error in DELETE /api/admin/advises:', err);
        return NextResponse.json({ error: err.message || 'Error interno del servidor' }, { status: 500 });
    }
}
